...
<!-- pollRead.jsp에서 넘어오는 투표한 항목을 받아서 득표수를 증가시켜 테스트 파일에 저장한다.  -->
...
//	투표 데이터가 넘어왔나(null 또는 공백이 아닌가?) 검사한다. => 반드시 null 검사를 먼저한다. 
	if (temp != null && temp.trim().length() != 0) {
		...
		if (result >= 1 && result <= itemCount) {
			...
			// poll.set(index, String.fomat("%s", result));
		...
		// poolResd.jsp로 돌려보낸다. 
			...
			// 넘어온 투표 데이터가 숫자가 아니므로 오류 메시지를 출력하고 poolRead.jsp로 돌려보낸다. 
			out.println("<script>"); 
...
//		스크립트가 실행된다. 
//		response.sendRedirect("pollRead.jsp"); 메소드는 인수로 지정한 페이지로 넘겨준다. 
